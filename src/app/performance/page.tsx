  ...
  if (!data?.hasData) return (
    <div className="flex flex-col gap-6 p-6">
      ...
      <div className="rounded-xl border border-zinc-800 bg-zinc-900/40 px-6 py-16 text-center">
        <p className="text-sm text-zinc-600">{data?.message ?? 'No positions yet. Run the autopilot to start building a portfolio.'}</p>
      </div>
    </div>
  )
            ...
            <tbody className="divide-y divide-zinc-800/50 bg-zinc-900/20">
              {positions.map(p => (
                <tr key={p.ticker} className="hover:bg-zinc-800/30 transition-colors">
                  ...
                  <td className={cn('px-4 py-3 font-mono text-xs', (p.mosAtPurchase ?? 0) >= 30 ? 'text-emerald-400' : 'text-amber-400')}>
